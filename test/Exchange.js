...
describe('Exchange', () => {
	...
	beforeEach(async () => {
		...
		token1 = await Token.deploy('Lasdex Token', 'LDX', '1000000')
	...
	describe('Order actions', async () => {
		...
		beforeEach(async () => {
			...
			// Make an order: they have token1 and they want to get token2
			transaction = await exchange.connect(user1).makeOrder(token2.address, amount, token1.address, amount)
			result = await transaction.wait()
		})
		...
		describe('Filling orders', async () => {
			...
			describe('Success', async () => {
				...
				it('executes the trade and charge fees', async () => {
					...
					expect(await exchange.balanceOf(token2.address, user2.address)).to.equal(tokens(0.999))
					expect(await exchange.balanceOf(token2.address, feeAccount.address)).to.equal(tokens(0.001))
				})
